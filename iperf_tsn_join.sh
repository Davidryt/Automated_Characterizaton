...
sshpass -p "$iperf_server_password" ssh "$iperf_server_user@$iperf_server_host" "nohup iperf3 -s -p 5000 -1 --logfile $server_results_folder/iperf_server_output_tsn.txt > /dev/null 2>&1 &"
...
for rate in "${rates[@]}"; do
    ...
    iperf_command="iperf3 -c 10.0.1.7 -u -p 5000 --cport 10000 -b ${rate}M -i 1 -t 30"
...
sshpass -p "$iperf_server_password" scp "$iperf_server_user@$iperf_server_host:$server_results_folder/iperf_server_output_tsn.txt" /home/netcom-i11-16/Downloads/results/
if [ $? -eq 0 ]; then
